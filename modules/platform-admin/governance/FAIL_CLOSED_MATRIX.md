...
- Core service token is missing or expired
- Core API call times out (read: 8 seconds, write: 20 seconds)
...
**System Action**: Deny request. Log failure with correlation ID. Return safe error to UI. Do NOT retry non-idempotent operations.
...
| **Publish Predefined Template**              | Valid Suite UI token; User role = platform_admin OR developer_ops; Template ID provided; Suite organizationId provided       | Validate token; Check RBAC; Validate template exists in Suite codebase; Resolve Suite org → Core org mapping; Call Core template publishing endpoint with Core service token, coreOrgId, correlation ID | Token invalid; Insufficient role; Template not found; Mapping resolution fails; Core API call fails (timeout, 401, 403, 500); Audit log write fails               | DENY. Return 401/403/404/502. Log failure. Do NOT retry if Core API returns 4xx. If Core API returns 5xx or timeout, log failure and return safe error. Audit log MUST record failure. |
...
**System Action**: Log failure. Return error to UI. Do NOT retry. If Core service token is expired, BFF MAY refresh token and retry ONCE.
